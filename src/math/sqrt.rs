...
pub fn sqrt(x: f64) -> f64 {
    cfg_if! {
        if #[cfg(target_feature = "sse2")] {
            ...
            // On wasm32 and Arm targets with 64-bit hard FPU, we know that
            // LLVM's intrinsic will compile to an optimized native instruction,
            // so we can leverage this for both code size and speed.
            if x < 0.0 {
                f64::NAN
            } else {
